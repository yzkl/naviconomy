...
class Refill(Base):
    ...
    odometer: Mapped[float] = mapped_column(Float(1))
    liters_filled: Mapped[float] = mapped_column(Float(2))
    brand_id: Mapped[int] = mapped_column(ForeignKey("dim_brands.id"))
    octane_id: Mapped[int] = mapped_column(ForeignKey("dim_octanes.id"))
    ethanol_percent: Mapped[float] = mapped_column(Float(2))
    cost: Mapped[float] = mapped_column(Float(2))
